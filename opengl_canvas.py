...
class Advanced3DCanvas(OpenGLFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        # Lighting
        self._lighting_enabled = False  # Start with lighting off for better point visibility
    ...
    def _setup_bindings(self):
        ...
        self.bind("<FocusIn>", lambda e: None)  # Accept focus
    ...
    def initgl(self):
        ...
        try:
            ...
            # Initialize lighting (but disable it for points initially)
            self._lighting_enabled = False  # Start with lighting off for better point visibility
            ...
            try:
                ...
                glEnable(GL_DEPTH_TEST)
                glPointSize(4.0)
                print("✓ Basic OpenGL setup completed")
            except:
                print("✗ Even basic OpenGL setup failed")
    ...
    def redraw(self):
        ...
        try:
            ...
            # Always draw coordinate system and grid for reference
            if self._show_axes:
                self._draw_axes()
            ...
            # Draw main data if available
            if self._points.size > 0:
                self._draw_data()
                # Debug output for first few frames
                if self._frame_count < 5:
                    print(f"Frame {self._frame_count}: Drawing {len(self._points)} points")
            else:
                # Draw a simple indicator when no data is loaded
                self._draw_no_data_indicator()
            ...
            # Try to at least clear the screen
            try:
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
            except:
                pass
    ...
    def set_points(self, points: np.ndarray, colours: np.ndarray):
        ...
        try:
            print(f"Setting points: {len(points)} points, {len(colours)} colors")
            ...
            if points.shape[0] != colours.shape[0]:
                raise ValueError(f"points ({points.shape[0]}) and colours ({colours.shape[0]}) must have the same length")
            ...
            if colours.shape[1] != 3:
                raise ValueError(f"Colors must be RGB (3 components), got shape {colours.shape}")
            
            # Normalize points to fit in [-1, 1] cube for better viewing
            if points.size > 0:
                ...
                if max_range > 0:
                    ...
                    print(f"Points normalized: range {max_range:.3f}, scale {scale:.3f}")
                else:
                    self._points = points.astype(np.float32)
                    print("Points have zero range, no normalization applied")
            else:
                self._points = points.astype(np.float32)
            
            # Ensure colors are in [0, 1] range
            if colours.max() > 1.0:
                print("Colors appear to be in [0, 255] range, normalizing to [0, 1]")
                self._colours = (colours / 255.0).astype(np.float32)
            else:
                self._colours = colours.astype(np.float32)
            
            print(f"Final points range: {self._points.min(axis=0)} to {self._points.max(axis=0)}")
            print(f"Final colors range: {self._colours.min(axis=0)} to {self._colours.max(axis=0)}")
            ...
            # Reset frame counter for debug output
            self._frame_count = 0
    ...
    def _draw_no_data_indicator(self):
        """Draw a simple indicator when no data is loaded"""
        glDisable(GL_LIGHTING)
        glPointSize(8.0)
        glColor3f(0.5, 0.5, 0.5)
        
        # Draw a simple cross pattern to indicate the canvas is working
        glBegin(GL_POINTS)
        for i in range(-2, 3):
            for j in range(-2, 3):
                if abs(i) + abs(j) <= 2:
                    glVertex3f(i * 0.1, j * 0.1, 0.0)
        glEnd()
