...
def Export_Spine_as_Text(tiff_path_output_txt, matrix, data_in_spine):
    """Save metadata *and* the full 3-D matrix to a CSV file, then show a quick
    2-D projection so users get immediate visual feedback.

    The output layout:
        # comment lines with metadata …
        layer,index,i,j,value   (one row per voxel ≠ 0)

    This keeps the file compact while still storing every occupied voxel.  If
    you prefer a dense slice-by-slice grid representation change the inner
    writer loop accordingly.
    """
    ...
    with open(tiff_path_output_txt, "w", newline="") as fp:
        # --------------- Metadata (commented) -------------------------
        fp.write("# Standardization file Generated by NEURON Simulator 2025\n")
        fp.write("# Reading instructions \n# 0: empty \n# 1: segment \n# 2: connection\n")

        for tag, attr in data_in_spine.tag_dict.items():
            fp.write(f"# {tag}: {getattr(data_in_spine, attr)}\n")

        # --------------- Matrix dump in [[layer] [layer] ...] format --
        fp.write('# Image:\n')
        fp.write('[\n')
        for k in range(depth):
            fp.write('[\n')  # open this layer
            for i in range(height):
                row_txt = ','.join(str(val) for val in matrix[k][i])
                fp.write(f"{row_txt}\n")
            fp.write(']')  # close this layer
            if k < depth - 1:
                fp.write(',\n')  # comma separator between layers
            else:
                fp.write('\n')
        fp.write(']\n')

    print("CSV written to", tiff_path_output_txt)
    ...
    try:
        ...
        # Maximum intensity projection for a fast 2-D preview
        np_mat = _np.array(matrix)
        mip = np_mat.max(axis=0)
        # _plt.figure("Spine preview")
        # _plt.imshow(mip, cmap="gray", origin="lower")
        # _plt.title("Max-projection (values: 0 blank, 1 segment, 2 connection)")
        # _plt.axis("off")
        # _plt.show(block=False)
    except Exception as _e:
        # Non-fatal – continue silently if display backend unavailable
        print("(preview skipped:", _e, ")")
...
def Geneate_Estimations(spine_number, base_name, data_in_spine):
    ...
    # ------------------------------------------------------------------
    # (moved down) we cannot attach the spine_matrix until it is actually
    # allocated below – we'll do it right after creating spine_3D.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # NOW that spine_3D exists attach it to the DataInImage so Qt viewer
    # and other callers can access it safely.
    # ------------------------------------------------------------------
    data_in_spine.spine_matrix = spine_3D  # numpy-style list of lists; (k,i,j)
    data_in_spine.bbox_min_ijk = tuple(min_ijk)  # (i_min, j_min, k_min)
